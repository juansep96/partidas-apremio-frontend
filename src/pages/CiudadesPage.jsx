...
import { ciudadesApi, dsApi } from '../api/client';
...
const initialFormData = { ciudad: '' };

export default function CiudadesPage({ desarrolloSocial = false }) {
  ...
  const api = desarrolloSocial ? dsApi.ciudades : ciudadesApi;
  ...
  const loadCiudades = useCallback(async () => {
    ...
    try {
      ...
      sileo.error({ title: 'Error al cargar ciudades', description: err.message });
    } finally {
      setLoading(false);
    }
  }, [user, canAccess, api, page, perPage, appliedSearch]);
  ...
  useEffect(() => {
    ...
    loadCiudades();
  }, [user, canAccess, navigate, desarrolloSocial, loadCiudades]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const payload = { ciudad: formData.ciudad.trim() };
      if (modal === 'create') {
        await api.create(payload);
        sileo.success({ title: 'Ciudad creada' });
      } else {
        await api.update(modal.id, payload);
        sileo.success({ title: 'Ciudad actualizada' });
      }
      closeModal();
      loadCiudades();
    } catch (err) {
            ...
            <div className="calles-pagination">
              <div className="calles-pagination-nav">
                ...
                <span className="calles-pagination-info">
                  Pág. {meta.current_page} de {meta.last_page} ({meta.total} ciudades)
                </span>
                <button
                  type="button"
                  className="calles-pagination-btn"
          ...
          <div className="admin-modal-overlay" onClick={closeModal}>
            ...
            >
              <header className="admin-modal-header">
                <h2>{modal === 'create' ? 'Nueva ciudad' : 'Editar ciudad'}</h2>
